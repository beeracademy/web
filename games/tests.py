...
class ApiTest(TestCase):
    ...
    def test_register_chug(self):
        Card.objects.create(game=Game.objects.get(id=self.game_id), value=14, suit="S")

        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.t2)
        r = self.client.post(
            f"/api/games/{self.game_id}/register_chug/",
    ...
    def test_end_game(self):
        ...
        for value, suit in game.get_all_cards():
            Card.objects.create(game=game, value=value, suit=suit)
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.t2)
        r = self.client.post(
            f"/api/games/{self.game_id}/end_game/",
            {"description": "foo", "end_datetime": datetime.datetime.now().isoformat()},
        )
        self.assertEqual(r.status_code, 200)
